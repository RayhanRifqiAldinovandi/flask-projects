...
def load_user(user_id):
	return UserString.query.get(int(user_id))
...
def register():
    if request.method == 'POST':
     ...
     cur = conn.cursor() 
     #insert the data to our users table
     ...
     if check :
         #will be executed if the username already exists 
       nope = "This username already exists, please enter a new one"
     else:
...
def delete(article_id):
    ...
    retrieve = 'SELECT * FROM articles  WHERE user_id = %s' % article_id
